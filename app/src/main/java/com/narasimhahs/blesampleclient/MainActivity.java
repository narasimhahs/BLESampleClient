...
import com.narasimhahs.blesampleclient.databinding.ActivityMainBinding;
import com.narasimhahs.blesampleclient.databinding.ViewGattServerBinding;
...
import static com.narasimhahs.blesampleclient.Constants.SCAN_PERIOD;
import static com.narasimhahs.blesampleclient.Constants.SERVICE_UUID;
...
public class MainActivity extends AppCompatActivity implements SeekBar.OnSeekBarChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mBinding.ledWhiteSeekbar.setMax(1023);
        mBinding.ledYellowSeekbar.setMax(1023);
        mBinding.ledWhiteSeekbar.setOnSeekBarChangeListener(this);
        mBinding.ledYellowSeekbar.setOnSeekBarChangeListener(this);
    }
    ...
    private void startScan() {
        ...
        //TODO - Removed filters to scan for all devices
     ...
     * @param input -
     */
    public void writeDataToCharacteristic(BluetoothGattCharacteristic readWriteCharacteristic, String input) {
        try {
            if (mGatt != null) {
                readWriteCharacteristic.setValue(input);
                mGatt.writeCharacteristic(readWriteCharacteristic);
                //Log.d("BLE_LIFECYCLE", "BLEConnectionHandler: writeDataToCharacteristic from device " + mBleDevice.getAddress() + " , mBluetoothGatt = " + mBluetoothGatt);
            } else {
                //Log.e("BLE_LIFECYCLE", "BLEConnectionHandler: not writing DataToCharacteristic");
                logError("Failed to write data");
            }

        } catch (Exception e) {
            Log.e("BLE_LIFECYCLE", "SOURCE_BLE Write Data Exception = " + e);
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (seekBar.getId() == R.id.led_white_seekbar) {
            command += "C1/" + seekBar.getProgress();
        } else if (seekBar.getId() == R.id.led_yellow_seekbar) {
            command += "C2/" + seekBar.getProgress();
        }

        BluetoothGattCharacteristic characteristic = BluetoothUtils.findLedCharacteristic(mGatt);
        if (characteristic == null) {
            logError("Unable to find LED characteristic.");
            disconnectGattServer();
            return;
        }

        writeDataToCharacteristic(characteristic, command);
    }
